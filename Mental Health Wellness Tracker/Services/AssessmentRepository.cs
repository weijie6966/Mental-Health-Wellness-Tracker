...
namespace Mental_Health_Wellness_Tracker.Services
{
    public class AssessmentRepository : IAssessmentRepository
    {
            ...
            try
            {
                ...
                var firestorePayload = new
                {
                    fields = new
                    {
                        ...
                        // Carry over the profile image (if available) so community posts show avatars
                        profileImagePath = new { stringValue = await ResolveProfileImagePathAsync(localEntry.UserId) },
        ...
        private async Task<string> ResolveProfileImagePathAsync(string userId)
        {
            await InitAsync();
            ...
            if (localProfile != null && !string.IsNullOrWhiteSpace(localProfile.ProfileImagePath))
            {
                return localProfile.ProfileImagePath;
            }

            var cloudProfile = await FetchUserProfileFromCloudAsync(userId);
            return cloudProfile?.ProfileImagePath ?? string.Empty;
        }
